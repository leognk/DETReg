...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2):
    ...
    return loss.sum(2) / loss.shape[1] / num_boxes

class SetIndividualCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_bbox = loss_bbox.sum(1) / num_boxes
        ...
        all_loss_bbox = torch.zeros(preds.shape[:-1], device=preds.device)
        all_loss_bbox[idx] = loss_bbox

        all_loss_giou = torch.zeros_like(all_loss_bbox)
        all_loss_giou[idx] = loss_giou

        return {'loss_bbox': all_loss_bbox, 'loss_giou': all_loss_giou}
...
def build_model_with_individual_criterion(args):
    ...
    if args.model == 'deformable_detr':
        ...
        criterion = SetIndividualCriterion(num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha)
...
def compute_individual_losses(criterion, outputs, targets):
